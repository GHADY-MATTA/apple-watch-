// src/components/MonthlyInsights.jsx
...
const MonthlyInsights = () => {
  ...
  useEffect(() => {
    const fetchInsights = async () => {
      try {
        const res = await axios.get('http://127.0.0.1:8000/api/monthly-insights', {
          headers: {
        ...
        console.error(err);
        setError('Failed to fetch monthly insights.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <p>⏳ Loading monthly insights...</p>;
  if (error) return <p style={{ color: 'red' }}>{error}</p>;
  if (!insights || insights.length === 0) return <p>No monthly insights available.</p>;

  return (
    <div style={{ padding: '1rem', border: '1px solid #ccc', borderRadius: '8px', marginTop: '1rem' }}>
      {insights.map((entry) => (
        <div key={entry.id} style={{ marginBottom: '1.5rem', backgroundColor: '#f4f8ff', padding: '1rem', borderRadius: '8px' }}>
          ...
          {entry.raw_json?.actionable_insights && (
            <div>
              <p><strong>🔍 Raw Focus Area:</strong> {entry.raw_json.actionable_insights.focus_area}</p>
