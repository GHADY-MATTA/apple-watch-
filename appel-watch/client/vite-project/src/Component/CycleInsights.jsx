...
const CycleInsights = () => {
  ...
  useEffect(() => {
    const fetchInsights = async () => {
      try {
        const res = await axios.get('http://127.0.0.1:8000/api/cycle-insights', {
          headers: {
        ...
        console.error(err);
        setError('Failed to fetch cycle insights.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <p>⏳ Loading cycle insights...</p>;
  if (error) return <p style={{ color: 'red' }}>{error}</p>;
  if (!insights || insights.length === 0) return <p>No cycle insights available.</p>;
